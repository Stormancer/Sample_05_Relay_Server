...
namespace Server.Plugins.API
{
    ...
    public class ControllerFactory<T> : IControllerFactory where T : ControllerBase
    {
        ...
        private async Task ExecuteRouteAction(Packet<IScenePeerClient> packet, Func<T, Packet<IScenePeerClient>,Task> action, string route)
        {
            using (var scope = _scene.DependencyResolver.CreateChild("request"))
            {
                ...
                catch (Exception ex)
                {
                    try
                    {
                        var handled = await controller.HandleException(new ApiExceptionContext(route, ex, packet));
                    ...
                    {
                        scope.Resolve<ILogger>().Log(LogLevel.Error, route, $"An exception occurred while executing exception handling method for '{route}' in controller '{controller.GetType().Name}'.", new AggregateException(ex, e));
                    }
        ...
        public void RegisterControllers()
        {
            ...
            foreach (var method in type.GetMethods())
            {
                ...
                else if (IsRawRoute(method))
                {
                    ...
                    _scene.AddRoute(procedureName, packet => ExecuteRouteAction(packet, action, procedureName), r => r);
                }
            }
        ...
        private bool IsRawRoute(MethodInfo method)
        {
            ...
            return method.ReturnType.IsAssignableFrom(typeof(Task)) && parameters[0].ParameterType == typeof(Packet<IScenePeerClient>);
        }
